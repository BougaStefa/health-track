...
/**
 * Service class that manages the business logic for Patient entities.
 * This class acts as an intermediary between the controller layer and the data access layer,
 * providing validation, error handling, and logging for all patient-related operations.
 * It ensures data integrity and proper business rule application before any database interaction.
 */
public class PatientService {
  ...
  /**
   * Adds a new patient to the system after performing validation checks.
   * Verifies that the patient object is not null and that a patient with the same ID
   * doesn't already exist in the database.
   *
   * @param patient The Patient object to be added
   * @throws IllegalArgumentException If the patient is null or if a patient with the same ID already exists
   ...
   * Validates that the patient object is not null before proceeding with the update.
   * If the patient doesn't exist, the DAO will handle the appropriate error.
   *
   ...
   * Note: This operation may fail if the patient is referenced by other records
   * (e.g., visits, prescriptions), which would violate referential integrity constraints.
   *
